...
class MigrationV7 {
  static Future<void> migrate(Database db) async {
    ...
    final tableInfo = await db.rawQuery(
      'PRAGMA table_info(${ArtistMetadataTable.tableName})',
    );
    ...
    if (!columnNames.contains('fetched_image_url')) {
      await db.execute(
        'ALTER TABLE ${ArtistMetadataTable.tableName} ADD COLUMN fetched_image_url TEXT',
      );
    }
    ...
    if (!columnNames.contains('fetch_attempted_at')) {
      await db.execute(
        'ALTER TABLE ${ArtistMetadataTable.tableName} ADD COLUMN fetch_attempted_at INTEGER',
      );
    }
